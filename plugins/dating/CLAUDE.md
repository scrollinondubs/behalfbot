...
The four rules below are wired to `chassis.config.yaml > modules.dating.safety_floor` and apply whenever the plugin is enabled. Profiles rejected by screening are simply not engaged further - ordinary silent screening, the same as any dating-app pass.

1. **Behaviour-based catfish screening, plus an OPTIONAL installer-configured regional gate (ships disabled, empty list).** The primary screening signals are behavioural: reverse-image consensus hits, the photo set on aggregator or adult sites, a digital footprint concentrated exclusively on a single country's internet with zero corroborating presence anywhere else, no verifiable current local presence, refusal of any verification step. Identity attributes (nationality, ethnicity, language, writing system, flag emoji) are never screening signals. Separately, `regional_default_reject` is a default-reject-with-override-gate mechanism the installer may enable and populate (`country_codes`) for their own threat model - the chassis ships it `enabled: false` with an empty list and names no countries. When configured, its triggers are: bio states the profile is located in or operating from a configured country, or PimEyes hits land exclusively on a configured country's domains and aggregators. **Override gate** (a gated profile passes only when ALL are true): photo verification clean + at least one corroborating digital footprint outside the single-country cluster (LinkedIn under her real name / locally-tagged IG / press / verifiable employer or university) + bio shows current verifiable local presence beyond touristy markers + standard scoring passes. Mixed signals → escalate to the installer with full evidence; do not auto-pass or auto-reject. The installer owns the country list; the override-gate evidence requirements are fixed.

2. **Reply-gated photo verification.** Photo verification runs only AFTER the match replies to the agent's opener - not at match-time and not pre-opener. Rationale: most catfish profiles are bot-like and never reply, so verification cost is wasted on them; verifying only on reply minimizes information given to non-replying scam profiles while still gating any further engagement (concierge framing reveal, installer facts, scheduling) behind a clean photo check. When a match replies for the first time, run `${CHASSIS_HOME}/plugins/dating/scripts/verify-new-match.sh <match-id>` (parallel TinEye + Google Lens + PimEyes + Yandex) BEFORE composing the reply. **Auto-reject signal must be byte-level / high-confidence:** TinEye exact-match on any adult-aggregator domain, Lens-identified celebrity, Lens high-confidence hit on adult-aggregator domain, or a PimEyes footprint concentrated exclusively on a single country's domains and aggregators with zero corroborating presence anywhere else (`pimeyes_single_country_footprint_only`; the old config key `pimeyes_russian_internet_only_presence` is a deprecated alias - if an install's config still sets it, honor its value and post a deprecation warning to the social channel telling the installer to rename it). **Yandex is similarity-only - never sufficient to auto-reject.** A "looks similar" Yandex hit (different person, similar aesthetic) is not catfish signal; there's always a similar-looking person on the web. Yandex contributes at most a YELLOW (escalate to the installer for review) when its hit is on a high-suspicion adult-aggregator domain - and even that requires the installer's call. Yandex keyword/distinct-name signals are INFORMATIONAL only: logged in raw.json, no longer affect the verdict. Pass to reply only when the byte-level / high-confidence engines (TinEye, Lens, PimEyes) are clean. Mixed signals → escalate to the installer with full evidence; do not respond. Log auto-rejects to `logs/dating/auto-rejects-YYYY-MM-DD.json` with full evidence; no Discord notification on auto-rejects (avoid alert fatigue). The regional gate (rule #1 above), when the installer has enabled and configured it, STILL runs at match-time - it's a cheap profile-text scan, not the costly photo-verification step.
